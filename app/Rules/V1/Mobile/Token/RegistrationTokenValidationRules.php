...
final class RegistrationTokenValidationRules implements ValidationRule
{
    public function validate(string $attribute, mixed $value, Closure $fail): void
    {
        ...
        if (Route::input(key: 'customer')['status'] !== CustomerStatus::Pending->value) {
            $fail('This account is '.Route::input(key: 'customer')['status'].'.');
        } elseif ($token['token'] === null) {
            $fail('Invalid verification token.');
        } elseif (! $token['token'] === $value) {
            $fail('The customer token is invalid.');
