...
final class LinkNewAccountCancellationRequest
{
    ...
    public function execute(Customer $customer, string $linked_account, array $request): array
    {
        return Http::withHeaders(['Content-Type' => 'application/vnd.api+json', 'Accept' => 'application/vnd.api+json'])
            ->post(url: $this->service->base_url.$customer->resource_id.'/linked-accounts/'.$linked_account.'/cancellations', data: $request)
            ->json();
    }
}
