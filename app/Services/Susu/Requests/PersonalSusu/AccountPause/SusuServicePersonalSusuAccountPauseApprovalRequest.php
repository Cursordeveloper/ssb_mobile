...
final class SusuServicePersonalSusuAccountPauseApprovalRequest
{
    ...
    public function execute(Customer $customer, string $susu, string $account_pause, array $request): array
    {
        return Http::withHeaders(['Content-Type' => 'application/vnd.api+json', 'Accept' => 'application/vnd.api+json'])
            ->post(url: $this->service->base_url.'customers/'.$customer->resource_id.'/personal-susus/'.$susu.'/collections/'.$account_pause.'/approvals', data: $request)
            ->json();
    }
}
